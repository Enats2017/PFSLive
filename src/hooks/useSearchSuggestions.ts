...
import { SuggestionItem, suggestionService } from '../services/followerScreenService';
...
const useSearchSuggestions = (
    ...
    const handleSearch = useCallback((text: string) => {
        ...
        timer.current = setTimeout(async () => {
            try {
                ...
                setSuggestions(results.filter(r => tabFilters.includes(r.tab)));
            } catch {
                setSuggestions([]);
            } finally {
                setLoading(false);
    ...
    return { query, suggestions, loading, dropdownVisible, handleSearch, clearSuggestions };
};
